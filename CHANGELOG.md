...
- `DuplicateOrderError` (an `ExchangeError`, **not** an `InvalidOrderError`): the venue refused a
  ...
  with "Duplicate order sent." (other `-2010` messages stay insufficient balance) and USDT-M
  futures `-4116` (`DUPLICATED_CLIENT_ORDER_ID`), Bitget `40786` plus `40708`, `43118`, `45034`
  and `50060` (the same error-code table's duplicate `clientOid` / `clientOrderId` rows), Upbit
  `duplicated_identifier`, Korbit `DUPLICATE_CLIENT_ORDER_ID`. Bithumb documents no such error,
  so it is unchanged. OKX `51784` ("Client order ID is being processed") is documented only for
  the finance flexible-loan endpoints, not `trade/order`, so it stays a plain `ExchangeError`.
...
- Korbit: `create_order(..., client_order_id=...)` is sent as `clientOrderId`
  ...
  up by `clientOrderId`. Exactly one of `order_id`/`client_order_id` is required
  (previously `NotSupportedError`). The order parser echoes `clientOrderId`.
- Korbit: launched KRW markets' `public_rules` now carries `price_tick_ladder`,
  read from the public `GET /v2/tickSizePolicy` (one call per market, `fetch_markets`
  makes 1 + N public requests), plus the Upbit-shaped rule keys (`min_notional`,
  `max_notional`, units, sources). A market whose policy call fails gets no ladder.
  Korbit publishes no order-quantity step, so `amount_step` stays `None`.
